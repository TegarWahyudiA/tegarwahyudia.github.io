...
class TbProgress extends CWidget
{
	...
	{
		if(!$this->type){
			if($this->percent > 100) $this->type='danger';
			elseif($this->percent > 90) $this->type='success';
			elseif($this->percent > 70) $this->type='warning';
			else $this->type='danger';
		}
...
//			$this->percent = 100;
	...
	public function run()
	{
		...
		echo '<div class="bar" style="width: '.($this->percent>100?100: $this->percent).'%;" title="'.$this->percent.'%">'.($this->label?$this->percent.'%':'').'</div>';
		echo '</div>';
	}
}
