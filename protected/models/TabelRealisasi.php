...
class TabelRealisasi extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('id_kegiatan, id_pegawai, tgl', 'required'),
			array('id_kegiatan, jml_realisasi', 'numerical', 'integerOnly'=>true),
			array('id_pegawai', 'length', 'max'=>20),
			array('keterangan, acc_on', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, id_kegiatan, id_pegawai, tgl, jml_realisasi, keterangan, created_on, acc_on', 'safe', 'on'=>'search'),
		);
	}
	...
	{
		return strftime("%d %b %Y",strtotime($this->tgl));
	}
	
	public function getStr_acc()
	{
		return $this->acc_on? strftime("%d %b %Y",strtotime($this->acc_on)) : '-';
	}
