...
class TabelTargetPegawai extends CActiveRecord
{
	/**
...
//			'kegiatan_fungsional'=>array(self::BELONGS_TO,'TabelKegiatanFungsional','id_kegiatan'),
	...
	{
		return Yii::app()->db->createCommand("SELECT SUM(jml_target) FROM ".TabelTargetMingguan::model()->tableName()." WHERE id_kegiatan=".$this->id_kegiatan.' AND id_pegawai='.$this->id_pegawai)->queryScalar();
	}

	public function getMingguan()
	{
		return TabelTargetMingguan::model()->findAll(array('condition'=>'id_kegiatan='.$this->id_kegiatan.' AND id_pegawai='.$this->id_pegawai, 'order'=>'mingguke'));
	}

	public function getRealisasi()
	{
		return TabelRealisasi::model()->findAll(array('condition'=>'id_kegiatan='.$this->id_kegiatan.' AND id_pegawai='.$this->id_pegawai, 'order'=>'tgl,jml_realisasi'));
	}

	public function getJml_realisasi()
	{
		return Yii::app()->db->createCommand("SELECT SUM(jml_realisasi) FROM ".TabelRealisasi::model()->tableName()." WHERE id_kegiatan=".$this->id_kegiatan.' AND id_pegawai='.$this->id_pegawai)->queryScalar();
	}

	public function getJml_realisasi_acc()
	{
		return Yii::app()->db->createCommand("SELECT SUM(jml_realisasi) FROM ".TabelRealisasi::model()->tableName()." WHERE id_kegiatan=".$this->id_kegiatan.' AND id_pegawai='.$this->id_pegawai.' AND acc_on IS NOT NULL')->queryScalar();
	}
