...
$prev_bulan = ($bulan==1)? array('rekap','tahun'=>$tahun-1,'bulan'=>12) : array('rekap','tahun'=>$tahun,'bulan'=>$bulan-1);
$next_bulan = ($bulan==12)? array('rekap','tahun'=>$tahun+1,'bulan'=>1) : array('rekap','tahun'=>$tahun,'bulan'=>$bulan+1);
...
$this->widget('TbMenu', array(
	...
	'items'=>array(
		...
		array('label'=>'Rekap Bulanan','url'=>'#','active'=>true),
...
//else
//	$tgl_2 = date('Y-m-d',strtotime( '-1 days' ));
...
foreach($dataProvider->getData() as $pegawai){
	...
	echo '<tr class='.$class.' title='.$pegawai->id_presensi.'><td align=center>'.($i++).'</td>
		<td>'.CHtml::link($pegawai->nama_pegawai,array('detail','id'=>$pegawai->id,'bulan'=>$bulan,'tahun'=>$tahun)).'</td>
		<td>'.($pegawai->id_golongan?$pegawai->golongan->golongan:'').'</td>';
...
<?php
if(!$excel){
	echo CHtml::link('<i class="g g-download"></i> Export Rekap',array('monitoring/rekap_absensi','bulan'=>$bulan,'tahun'=>$tahun,'excel'=>'yes'));
	echo CHtml::link('<i class="g g-download"></i> Detail Harian',array('harian','bulan'=>$bulan,'tahun'=>$tahun,'excel'=>'yes'),array('style'=>'float:right'));
}
?>
